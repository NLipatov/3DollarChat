...
public class FfmpegConverter : IAsyncDisposable
{
    ...
    private async Task SendFormData(HttpClient httpClient, byte[] segmentContent, string segmentName)
    {
        using var formData = new MultipartFormDataContent();
        var fileContent = new ByteArrayContent(segmentContent);
        formData.Add(fileContent, "payload", segmentName);

        var endpointAddress = string.Join("", _navigationManager.BaseUri, "hlsapi/store");
        Console.WriteLine($"Posting to: {endpointAddress}");
        await httpClient.PostAsync(endpointAddress, formData);
    }
